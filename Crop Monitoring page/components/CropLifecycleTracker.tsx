      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 mb-5 border-b border-white/50">
        <div>
          <h3 className="text-lg font-bold text-zinc-800 flex items-center gap-2">
            <span>🌱</span> Crop Lifecycle Progression
          </h3>
          <p className="text-xs sm:text-sm text-zinc-500">
            Full physiological development — germination to harvest
          </p>
        </div>
        <div className="flex items-center gap-3 text-xs font-medium">
          ...
          <span className="flex items-center gap-1.5 text-teal-700">
            <span className="w-2.5 h-2.5 rounded-full bg-teal-500 animate-pulse inline-block" /> Active
          </span>
          <span className="flex items-center gap-1.5 text-zinc-400">
            <span className="w-2.5 h-2.5 rounded-full bg-zinc-300 inline-block" /> Upcoming
          </span>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                <h4 className="font-bold text-sm text-zinc-800 leading-tight">{stg.name}</h4>
                <p className="text-[11px] text-zinc-500 mt-1">
                  {formatDateString(stg.startDate)} – {formatDateString(stg.endDate)}
                </p>
              </div>

              <div className="mt-3 pt-2.5 border-t border-white/40 text-[11px] space-y-1">
                <p className="text-zinc-600 truncate">
                  <strong className="text-zinc-700">Focus:</strong> {stg.nutrientFocus}
                </p>
                <p className="text-zinc-500 truncate">
                  <strong>Water:</strong> {stg.waterRequirement}
                </p>
              </div>
            </div>
          );
